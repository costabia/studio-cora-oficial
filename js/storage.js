...
export async function hydrateData(seed){const local=loadData(seed);if(!cloudConfigured())return local;try{const remote=await cloudLoad();if(remote){const original=structuredClone(remote);const clean=cleanData(remote);localStorage.setItem(KEY,JSON.stringify(clean));if(JSON.stringify(clean)!==JSON.stringify(original))await cloudSave(clean);return clean}await cloudSave(local);return local}catch(error){console.warn('Supabase indisponível; usando dados locais.',error);return local}}
